...
@Component
@Service
public class MyaddonOperationsImpl implements MyaddonOperations{
	private static Logger logger = Logger.getLogger(MyaddonOperations.class.getName());
	...
	public void annotateType(JavaType javaType) {
		...
		if (null == MemberFindingUtils.getAnnotationOfType(mutableTypeDetails.getAnnotations(), new JavaType(RooMyaddon.class.getName()))) {
			JavaType rooRooMyaddon = new JavaType(RooMyaddon.class.getName());
			if (!mutableTypeDetails.getAnnotations().contains(rooRooMyaddon)) {
				AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooMyaddon);
